...
def main():
   ...
   l = mdl.addVars(((item, element) for item in K for element in V + [NODES+1]), vtype = GRB.INTEGER, name = "l")
   b = mdl.addVars(((item, element) for item in E for element in V + [NODES+1]), vtype=GRB.CONTINUOUS, lb = 0, ub = 1, name = "b")
   ...
   mdl.addConstrs(((x_d[d,(0,j)])-(x_d[d,(j,NODES+1)]) == 0) for d in D for j in N)
   ...
   mdl.addConstrs(((quicksum(x_e[e,(i,j)] for i in V if i!=j)-quicksum(x_e[e,(j,i)] for i in V_bar if i!=j) == 0) for j in N for e in E), name='s')
   ...
   mdl.addConstrs((l[(e,j)]* x_e[e,(i,j)] ==  x_e[e,(i,j)]*(l[(e,i)]+q[j])) for e in E for j in V_bar for i in V if i!=j and not (i==0 and j==NODES+1)) #load upon arriving at j is equal to j's total load+all previous loads
   mdl.addConstrs((l[(d,j)]* x_d[d,(i,j)] ==  x_d[d,(i,j)]*(l[(d,i)]+q[j])) for d in D for j in V_bar for i in V if i!=j and not (i==0 and j==NODES+1)) #load upon arriving at j is equal to j's total load+all previous loads
   mdl.addConstrs((l[(k,0)] == 0) for k in K)


   mdl.addConstrs(((b[e,j]*x_e[e,(i,j)] == x_e[e,(i,j)] * (b[e,i]*(1-r[i])+r[i]-(a[(i,j)]/EV_velocity)*(gamma+gamma_l*l[(e,i)]))) for i in V for j in V_bar for e in E if i!=j and not (i==0 and j==NODES+1)), name='ssss') #tracks rem battery capacity for each intermediate node
   mdl.addConstrs((b[e,j]*x_e[e,(i,j)] >= x_e[e,(i,j)]*(a[(j,NODES+1)]/EV_velocity)*(gamma+gamma_l*l[(e,j)])) for i in N for j in V_bar for e in E if i!=j and not (i==0 and j==NODES+1)) #enough battery to go back to depot
   ...
   mdl.addConstrs(x_e[e,(j,NODES+1)] + x_e[e,(0,j)] <= 1 for e in E for j in N) 

   # To ensure one EV makes one trip
   mdl.addConstrs(quicksum(x_e[e,(j,NODES+1)] for j in N) <= 1 for e in E) 
   ...
   #BB
   mdl.addConstr(quicksum(p[i] for i in N) <= (quicksum((1-b[e,NODES+1])*260*EV_cost*x_e[e,(i,NODES+1)] for i in N for e in E)))
   ...
   mdl.setObjective((quicksum(x_d[d,(0,j)]*a[(0,j)]*2 for j in N for d in D ))*w_dv + (quicksum(x_e[e,(i,j)]*a[(i,j)] for i in V for j in V_bar for e in E if i!=j and not (i==0 and j==NODES+1)))*w_ev \
                  +  theta * ((quicksum((1-b[e,NODES+1])*260*EV_cost*x_e[e,(i,NODES+1)] for i in N for e in E)) - quicksum(p[i] for i in N))) 
