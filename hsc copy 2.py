...
st[0]=0
gamma = 0.2    #0.133                                                                 #battery depletion rate for EVs without any load
gamma_l =  0.2 #0.026                                                               #load dependent battery depletion rate for EVs
b = [(i,j) for i in V for j in E]                                           #battery level upon arriving at node i
T_max_EV = 550                                                              #max operation time per EV 
T_max_GV = 400                                                              #max operation time per GV
...
r = mdl.addVars((item for item in V), vtype=GRB.BINARY, name = "r")
...
mdl.addConstrs((quicksum(x_e[e,(j,0)]*z[e,j] for j in N) <= T_max_EV) for e in E)


mdl.addConstr(r[0]==1)
mdl.addConstrs(r[i]==0 for i in N)
...
mdl.setObjective((quicksum(x_d[d,(0,j)]*c[(0,j)]*1.5 for d in D for j in V if j!=0))+(quicksum(x_d[d,(j,0)]*c[(j,0)]*.100 for j in V for d in D if j!=0))+(quicksum(x_e[e,(i,j)]*c[(i,j)] for i in V for j in V for e in E if i!=j))+ 0.00001*quicksum(z[e,j] for e in E for j in N))
