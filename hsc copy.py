...
q = {i: rnd.randint(1,20) for i in N}                                       #demand for customers
...
T_max_EV = 350                                                                #max operation time per vehicle 
T_max_GV = 500
...
for item in x_e_result:
   if x_e_result[item]==1:
      temp = [int(match.group()) for match in re.finditer(r'\b\d+\b', item)]
      G.add_edge(temp[1], temp[2], color=temp[0]*10)
      s = f"l[{temp[0]},{temp[2]}]"
