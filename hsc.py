...
st[0]=0
gamma = 0.133/60    #0.133                                                   #battery depletion rate for EVs without any load (0.133 per min)
gamma_l =  0.026/60 #0.026                                                   #load dependent battery depletion rate for EVs   (0.026 per min per ton)
...
def cost_calculation(x_e_result,x_d_result,b0_result):
   ...
   for item in x_e_result:
      if x_e_result[item]>=0.99:
         temp = [int(match.group()) for match in re.finditer(r'\b\d+\b', item)]
         miles_EV = miles_EV + a[(temp[1],temp[2])]

   for item in x_d_result:
      if x_d_result[item]>=0.99:
         temp = [int(match.group()) for match in re.finditer(r'\b\d+\b', item)]
         miles_GV = miles_GV + a[(temp[1],temp[2])]
