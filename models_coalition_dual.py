...
class SubProblem:
    ...
    def partial_path(self, label,current_node):
        ...
        while U:
            ...
            if not is_dominated and current_label not in L[current_node]:
                    ...
                    for new_node in neigh:
                        ...
                        if new_node_converted not in self.adj[current_node_converted]:
                            continue
...
class MasterProblem:
    ...
    def relaxedLP(self, extended_set) -> None:
        ...
        e_BB = self.model.addVar(vtype=GRB.CONTINUOUS, name = "e_BB")
        for i in N:
            p[i] = self.model.addVar(vtype=GRB.CONTINUOUS, name = f"p{i}")
            e_IR[i] = self.model.addVar(vtype=GRB.CONTINUOUS, name = f"e_IR{i}")
            e_S[i] = self.model.addVar(vtype=GRB.CONTINUOUS, name = f"e_S{i}")
        ...
        #IR
        self.model.addConstrs((p[i] <= a[i, 0] * GV_cost * q[i] + a[i, 0] * GV_cost + e_IR[i] for i in N),name='IR')
        #self.model.addConstrs(((p[i]<=(a[(i,0)]/EV_velocity)*(gamma+gamma_l*q[i])*260*EV_cost+(a[(i,0)]/EV_velocity)*(gamma+gamma_l*0)*260*EV_cost) for i in N),name='IR')
